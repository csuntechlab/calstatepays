<template>
    <div class="homeSeals">
        <div class="container">
            <div class="row">
                <div class="col-md-12 mb-4">
                    <h2 class="text-center font-italic">
                        <span class="homepageUniversities__headline"> Endorsed by 7 CSUs,</span>
                        <span class="homepageUniversities__headline"> CalStatePays combines</span>
                        <span class="homepageUniversities__headline"> the data of</span>
                        <span class="homepageUniversities__headline"> SoCal's top public universities</span>
                    </h2>
                </div>
                <div class="row no-gutters">
                    <figure  v-for="(item, index) in universitySeals" :key="index" class="col-md col-sm-3 col-3">
                        <img :src="item.url" :alt="item.alt">
                        <figcaption class="text-center" >{{item.name}}</figcaption>
                    </figure>
                </div>  
            </div>
        </div>
    </div>
</template>

<script>
export default {
    name: "campus-seals",
    data() {
        return {
            url: "",
            isShowing: false,
            universitySeals:[
                {url: window.baseUrl + '/img/csuseals/long_beach_seal.svg',name:'Long Beach',alt:''},
                {url: window.baseUrl + '/img/csuseals/poly_seal.svg',name:'Pomona',alt:''},
                {url: window.baseUrl + '/img/csuseals/fullerton_seal.svg',name:'Fullerton',alt:''},
                {url :window.baseUrl+ '/img/csuseals/northridge_seal.svg',name:'Northridge',alt:''},
                {url: window.baseUrl + '/img/csuseals/dominguez_seal.svg',name:'Dominguez Hills',alt:''},
                {url: window.baseUrl + '/img/csuseals/los_angeles_seal.svg',name:'Los Angeles',alt:''},
                {url: window.baseUrl + '/img/csuseals/channel_islands_seal.svg',name:'Channel Islands',alt:''},                
            ]
        };
    },
}
</script>

